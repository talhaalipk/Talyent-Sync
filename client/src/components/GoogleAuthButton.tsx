...
export default function GoogleAuthButton({ role }: GoogleAuthButtonProps) {
  ...
  const login = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      if (tokenResponse.access_token) {
        const res = await googleAuth(tokenResponse.access_token, role || null);
        if (res?.user) {
          navigate("/dashboard");
        }
      }
  ...
  return (
    <button
      onClick={() => login()}
      className="w-full flex items-center justify-center gap-2 bg-white border rounded-lg py-2 px-4 shadow hover:bg-gray-50"
    >
      <img
